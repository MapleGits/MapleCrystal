...
public class MapleClient implements Serializable {
    ...
    public final void updateCharacterCards(final Map<Integer, Integer> cids) {
        ...
        try {
            ...
            PreparedStatement psu = con.prepareStatement("INSERT INTO `character_cards` (id, accid, worldid, characterid, position) VALUES (NULL, ?, ?, ?, ?)");
    ...
    private List<CharNameAndId> loadCharactersInternal(int serverId) {
        ...
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = con.prepareStatement("SELECT id, name, gm FROM characters WHERE accountid = ? AND world = ? AND hcMode != 2");
            ps.setInt(1, accId);
